...
def gen_secrets(channels: List[int], num_decoders: int = 8) -> bytes:
    ...
    for channel in channels:
        # Se genera una clave de 256 bits (32 bytes) para el canal
        channel_key = os.urandom(32)
        # Se codifica en base64 para facilitar el almacenamiento en JSON
    ...
    partial_keys = {}
    for decoder_id in range(1, num_decoders + 1):
        # Generar una clave aleatoria de 16 bytes para cada decodificador
        key_share = os.urandom(16)
        decoder_key_name = f"decoder_{decoder_id}"
        partial_keys[decoder_key_name] = base64.b64encode(key_share).decode('utf-8')
...
def parse_args():
    ...
    parser.add_argument("num_decoders", type=int, help="Número de decodificadores a generar.")
